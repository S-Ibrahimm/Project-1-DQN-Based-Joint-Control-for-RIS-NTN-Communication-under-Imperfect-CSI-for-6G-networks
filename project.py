...
class WirelessEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.power_lvls = [-10, -5, 0, 5, 10, 15, 20, 25, 30]
        self.modulations = ['BPSK' , 'QPSK', '16-QAM']
#[UPDATED WEEK 10] Sub-Array RIS Grouping: Instead of ON/OFF, we have 4 groups of 32 elements.
        # The agent chooses how many sub-arrays to actively align (0, 1, 2, 3, or 4).
        self.ris_active_subarrays = [0, 1, 2, 3, 4]
        
    # [NEW WEEK 8] Joint action space expanded to (Power, Modulation, RIS Phase Shift)
        self.action_list = [(p, m, r) for p in self.power_lvls for m in self.modulations for r in self.ris_active_subarrays]
...
# [NEW WEEK 9] Expanded observation range for NTN distances (10m to 3000m)
        ...
        self.eta_ris = 2.0
        self.d_bs_ris = 2500.0        # Fixed distance from ground zero(perp dist of NTN to ground) to RIS (meters)
...
# [NEW WEEK 10] Measurement Noise Standard Deviation for Imperfect CSI (in dB)
        self.csi_noise_std_db = 2.0

    # [UPDATED WEEK 10] Scale distance and NOISY channel gain into standard [0, 1] range for the agent
    def _get_normalized_state(self):
        # Add random Gaussian measurement noise to true channel gain to simulate Imperfect CSI
        noisy_chan_gain = self.chan_gain_db + np.random.normal(0.0, self.csi_noise_std_db)
        norm_dist = (self.distance - 10.0) / (3000.0 - 10.0)
        norm_gain = (noisy_chan_gain - (-150.0)) / (-30.0 - (-150.0))
...
# [UPDATED WEEK 10] Calculate array gain based on how many sub-arrays are turned ON
        # ris_mode is now a number from 0 to 4. Each active sub-array has 32 elements.
        active_elements = ris_mode * 32
...
def baseline_greedy(snr_db,env):
    ...
    if snr_db >= 15.0:
        target_tuple = (5, "16-QAM", 4)  # High SNR -> Low power, high modulation, RIS ON
    elif snr_db >= 8.0:
        target_tuple = (10, "QPSK", 4)   # Medium SNR -> Moderate power, QPSK, RIS ON
    else:
        target_tuple = (20, "BPSK", 4)   # Low SNR -> Max power, robust modulation, RIS ON

    # Fallback just in case the exact tuple isn't in the action list
    if target_tuple in env.action_list:
        return env.action_list.index(target_tuple)
    else:
        return 0

# [NEW WEEK 10] Classical Optimization Benchmark (Exhaustive Upper Bound)
def baseline_classical_optimal(env, current_distance, current_noisy_gain):
    """
    Baseline 4: Represents a classical mathematical solver. 
    It iterates through every possible action combination to find the mathematically 
    perfect choice (highest reward) for the current state.
    """
    best_action = 0
    max_reward = -float('inf')
    
    # Store the original environment state so we don't permanently alter it while searching
    original_snr = env.snr_db
    original_gain = env.chan_gain_db
    
    # The classical solver evaluates every single action manually
    for act_idx in range(env.action_space.n):
        p_tx_dbm, modulation, ris_mode = env.action_list[act_idx]
        
        # Manually compute what the environment would do
        test_gain_db = env._compute_channel_gain_db(current_distance, ris_mode=ris_mode)
        test_p_rx_dbm = p_tx_dbm + test_gain_db
        test_snr_db = test_p_rx_dbm - env.noise_floor
        
        test_ber = env._compute_ber(test_snr_db, modulation)
        test_data_rate = env._compute_data_rate(modulation)
        
        snr_gap = test_snr_db - env.target_snr
        
        if test_ber <= 0.001:
            test_reward = test_data_rate + 5.0 - (env.lam * max(0, p_tx_dbm))
        else:
            test_reward = snr_gap - (env.lam * max(0, p_tx_dbm))
            
        if test_reward > max_reward:
            max_reward = test_reward
            best_action = act_idx
            
    # Restore the true environment state after searching
    env.snr_db = original_snr
    env.chan_gain_db = original_gain
...
if __name__ == "__main__":  # boilerplate
    ...
    for step in range(1, 6):
         # Capture SNR before the DQN step alters the environment state
            ...
            current_distance = env.distance
            current_noisy_gain = eval_state[1] * (-30.0 - (-150.0)) + (-150.0) # Un-normalize for the classical solver
            # [NEW WEEK 10] 1. Classical Optimization (Stopwatch Start)
            start_time_classical = time.time()
            classical_act = baseline_classical_optimal(env, current_distance, current_noisy_gain)
            end_time_classical = time.time()
            classical_time_ms = (end_time_classical - start_time_classical) * 1000.0
            c_power, c_mod, c_ris = env.action_list[classical_act]
...
# [NEW WEEK 10] 3. DQN Agent selects and executes action (Stopwatch Start)
            start_time_dqn = time.time()
            dqn_action = agent.select_action(eval_state, evaluate=True)
            end_time_dqn = time.time()
            dqn_time_ms = (end_time_dqn - start_time_dqn) * 1000.0
           ...
           # Print the benchmark comparison
            print(f"--- Step {step} | Dist: {current_distance:.1f}m ---")
            print(f"Classical Math : [{c_power:2d} dBm, {c_mod:6s}, RIS:{c_ris}] | Time: {classical_time_ms:.3f} ms")
            print(f"Greedy Rule    : [{g_power:2d} dBm, {g_mod:6s}, RIS:{g_ris}]")
            print(f"DQN Agent      : [{chosen_power:2d} dBm, {chosen_mod:6s}, RIS:{chosen_ris}] | Time: {dqn_time_ms:.3f} ms")
            print(f"-> Agent Result: SNR: {env.snr_db:.1f}dB, BER: {ber:.4f}, Reward: {reward:.2f}\n")
